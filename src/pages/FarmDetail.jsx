...
import { ChevronRight, Bell, BellOff, Info, Store } from 'lucide-react';
...
export default function FarmDetail() {
  ...
  const handleSetAlert = () => {
    ...
    toast.success(t('alert_set_ok', { price: alertPrice }));
  };
  ...
  return (
    <div className="p-4 space-y-4 pb-28">
      <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black text-foreground">{t('farmdetail_title')}</h1>
          <p className="text-xs text-muted-foreground">{t('farmdetail_subtitle')}</p>
        </div>
      </motion.div>
      ...
      {holdings.length > 0 ? (
        <>
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.05 }}
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-xs text-muted-foreground">{t('monthly_income_total')}</p>
                <p className="text-4xl font-black text-primary">{totalMonthly.toFixed(1)} ₪</p>
              </div>
              <div className="text-left">
                <p className="text-xs text-muted-foreground">{t('total_investment')}</p>
            ...
            <div className="grid grid-cols-2 gap-2 pt-3 border-t border-primary/20">
              <div className="text-center">
                ...
                <p className="text-[10px] text-muted-foreground">{t('active_farms')}</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-black text-foreground">{holdings.reduce((s, h) => s + h.qty, 0)}</p>
                <p className="text-[10px] text-muted-foreground">{t('total_shares')}</p>
              </div>
            </div>
          </motion.div>
            ...
            <motion.div key={h.id} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 + i * 0.07 }}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{h.meta.location} · {t('yield_label2')} {h.meta.yield}%</p>
                  </div>
                </div>
                <div className="text-left">
                  <p className="text-xs text-muted-foreground">{t('monthly_income')}</p>
                  <p className="text-base font-black text-primary">+{h.monthlyIncome} ₪</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: t('shares_label'), value: h.qty },
                  { label: t('investment_label'), value: `₪${h.totalInvested.toFixed(0)}` },
                  { label: t('payback_label'), value: t('payback_months', { n: h.payback }) },
                ].map(s => (
              ...
              <div className="flex items-center gap-2 bg-primary/5 border border-primary/20 rounded-xl p-2.5">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {t('payback_info', { n: h.payback, roi: h.roiPct })}
                </p>
              </div>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }}
          ...
          <p className="text-base font-black text-foreground">{t('empty_portfolio')}</p>
          <p className="text-xs text-muted-foreground">{t('empty_portfolio_sub')}</p>
          <button onClick={() => navigate('/marketplace')}
            ...
            <Store className="w-4 h-4" /> {t('go_marketplace')}
          </button>
        </motion.div>
      )}
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}
        ...
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-bold text-muted-foreground">{t('smp_chart_title')}</p>
          {alertActive && (
            <div className="flex items-center gap-1 bg-accent/20 px-2 py-0.5 rounded-full">
              <Bell className="w-3 h-3 text-accent" />
              <span className="text-[10px] font-bold text-accent">{t('alert_at', { price: alertPrice })}</span>
            </div>
          )}
        </div>
        <ResponsiveContainer width="100%" height={100}>
          <AreaChart data={smpHistory}>
            ...
            <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 10, fontSize: 11 }} formatter={(v) => [`₪${v}`, 'SMP']} />
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.35 }}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-bold text-foreground">{t('price_alert_title')}</p>
          </div>
          {alertActive ? (
            <button onClick={() => { setAlertActive(false); toast(t('alert_removed')); }}
              className="flex items-center gap-1 text-xs text-destructive font-bold">
              <BellOff className="w-3.5 h-3.5" /> {t('alert_remove')}
            </button>
          ) : (
            <button onClick={() => setShowAlertForm(v => !v)} className="text-xs text-primary font-bold">
              {showAlertForm ? t('alert_cancel') : t('alert_set')}
            </button>
          )}
        </div>
        {alertActive && (
          <div className="bg-accent/10 border border-accent/30 rounded-xl p-3 flex items-center gap-2">
            <Bell className="w-4 h-4 text-accent" />
            <p className="text-xs text-foreground">{t('alert_active_desc', { price: alertPrice })}</p>
          </div>
        )}
        <AnimatePresence>
          {showAlertForm && !alertActive && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              ...
              <p className="text-xs text-muted-foreground">{t('alert_set_threshold')}</p>
              <div className="flex items-center gap-3">
                <button onClick={() => setAlertPrice(p => Math.max(0.30, +(p - 0.01).toFixed(2)))} className="p-2 rounded-xl bg-muted border border-border font-bold">−</button>
                <div className="flex-1 text-center bg-muted rounded-xl py-2">
                  <span className="text-xl font-black text-accent">₪{alertPrice}</span>
                </div>
                <button onClick={() => setAlertPrice(p => +(p + 0.01).toFixed(2))} className="p-2 rounded-xl bg-muted border border-border font-bold">+</button>
              </div>
              <button onClick={handleSetAlert} className="w-full py-3 bg-accent text-accent-foreground font-black rounded-2xl active:scale-95 text-sm">
                {t('alert_confirm', { price: alertPrice })}
              </button>
            </motion.div>
          )}
        </AnimatePresence>
