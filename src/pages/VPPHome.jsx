...
import { MessageSquare, ChevronLeft, Send, Sparkles } from 'lucide-react';

// --- רכיב וואטסאפ אישי ומניע לפעולה ---
function WhatsAppSection({ lang }) {
  const phoneNumber = "9725XXXXXXX"; // החלף למספר שלך בלי ה-0 בהתחלה
...
function ContactForm({ lang }) {
  return (
    <motion.div 
      ...
      className="mx-1 bg-card/60 border border-border/50 rounded-2xl p-6 space-y-4"
    >
      <div className="flex items-center gap-2 border-b border-border/50 pb-3">
        <Sparkles className="w-4 h-4 text-primary" />
        <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
          {lang === 'he' ? 'השארת פרטים למועדון' : 'Join the Club'}
        </h3>
      </div>
      ...
      <form action="https://formspree.io/f/YOUR_FORM_ID" method="POST" className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <input 
            ...
            className="w-full bg-background/40 border border-border/40 rounded-xl px-4 py-3 text-xs focus:ring-1 focus:ring-primary/30 outline-none" 
          />
          <input 
            type="tel" name="phone" 
        ...
        <textarea 
          name="message" rows="2"
          placeholder={lang === 'he' ? 'ספר לנו על המערכת שלך...' : 'Tell us about your system...'} 
          className="w-full bg-background/40 border border-border/40 rounded-xl px-4 py-3 text-xs focus:ring-1 focus:ring-primary/30 outline-none resize-none"
        ></textarea>
        
        <button 
          type="submit" 
          className="w-full py-3.5 rounded-xl font-black text-white text-xs flex items-center justify-center gap-2 shadow-lg shadow-primary/20 active:scale-95 transition-transform"
          style={{ background: 'linear-gradient(135deg, #10b981, #059669)' }}
        >
          <Send className="w-3.5 h-3.5" />
          {lang === 'he' ? 'שלח בקשת הצטרפות' : 'Submit Request'}
        </button>
      </form>
    </motion.div>
  );
...
// --- הזרקה לתוך ה-Component הראשי ---
export default function VPPHome() {
  // ... לוגיקה קיימת ...
  const { lang } = useLang();

  return (
    <div className="p-3 space-y-4 pb-28 text-right" style={{ direction: 'rtl' }}>
      {/* ... כל ה-Widgets הקיימים שלך ... */}

      {/* הבלוק האחוד של יצירת קשר */}
      <div className="space-y-4 border-t border-border/30 pt-6 mt-8">
        <WhatsAppSection lang={lang} />
        <div className="flex items-center gap-4 px-4">
           <div className="h-[1px] bg-border/50 flex-1"></div>
           <span className="text-[10px] text-muted-foreground font-medium italic">או בדרך המסורתית</span>
           <div className="h-[1px] bg-border/50 flex-1"></div>
        </div>
