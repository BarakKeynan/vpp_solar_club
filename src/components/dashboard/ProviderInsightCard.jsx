  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between mt-1 text-[9px]" style={{ color: 'rgba(255,255,255,0.3)' }}>
        <span>00:00</span>
        <span>השוואת תשואה לפי שעה</span>
        <span>24:00</span>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="text-center space-y-1">
        <p className="text-sm font-black text-white">Scanning Grid...</p>
        <p className="text-xs" style={{ color: 'rgba(147,197,253,0.7)' }}>מנתח נתוני Noga Data Hub · 90 יום</p>
      </div>
      {/* Scan bar */}
...
Write a SHORT Hebrew insight (2–3 sentences) explaining objectively WHY Cellcom Energy yields more for THIS specific production/consumption signature. Reference net-metering tariff and SMP rates. Be neutral and data-driven.`,
        response_json_schema: { type: 'object', properties: { insight: { type: 'string' } } },
      }).then(r => {
        setInsight(r.insight);
        setAnalyzing(false);
      }).catch(() => {
        setInsight('Cellcom Energy מציעה תעריף נטו-מטרינג גבוה ב-13% בשעות שיא הצהרים – שבהן הפרופיל שלך מייצא את עיקר האנרגיה. תעריף פריקת הסוללה בשעות הערב גבוה בכ-9% מ-IEC, מה שמגדיל את התשואה הכוללת משמעותית.');
        setAnalyzing(false);
      });
    }
  };
  ...
  const handleSwitch = async () => {
    ...
    setSwitching(false);
    setSwitched(true);
    toast.success('הבקשה נשלחה ל-Cellcom Energy! VPP Active Optimization הופעל.');
  };

  const winner = PROVIDERS.find(p => p.bestMatch);
  const iec = PROVIDERS.find(p => p.name.startsWith('IEC'));
      ...
      >
        <div
          ...
          style={{
            ...
            boxShadow: '0 0 14px 0 rgba(59,130,246,0.3)',
          }}
        >
          <Search className="w-5 h-5" style={{ color: '#93C5FD' }} />
        </div>
        <div className="flex-1 text-right">
          <p className="text-sm font-black text-white">Unlock My Personalized Yield Audit</p>
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(147,197,253,0.7)' }}>
            90 ימי נתוני ייצור · Noga Data Hub · תוצאה מיידית
          </p>
        </div>
        ...
        {open && (
          ...
          >
            ...
            >
              {/* Scanning overlay */}
              <AnimatePresence>
                {scanning && <ScanningOverlay onDone={handleScanDone} />}
              </AnimatePresence>
              ...
              <div className="px-4 pb-8 space-y-5">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-base font-black text-white">Your Signature Profit Audit</p>
                    <p className="text-[10px] mt-0.5" style={{ color: 'rgba(147,197,253,0.6)' }}>
                      מבוסס על 90 ימי נתוני מונה בפרסי 15 דקות · Noga Data Hub
                    </p>
                  </div>
                  <button
                    onClick={() => { setOpen(false); setReady(false); }}
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-black" style={{ color: '#FCD34D' }}>Synergy Bonus החודש</p>
                    <p className="text-[11px] mt-0.5" style={{ color: 'rgba(253,211,77,0.75)' }}>
                      תרומתך לסינדיקט חיפה הניבה לך <span className="font-black">+2.5%</span> נוסף השוות ₪21 נוספים
                    </p>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-xs font-bold mb-2" style={{ color: 'rgba(255,255,255,0.4)' }}>
                    השוואת תשואה לפי שעה (SMP vs תעריף)
                  </p>
                ...
                <div>
                  <p className="text-xs font-bold mb-2" style={{ color: 'rgba(255,255,255,0.4)' }}>
                    Yield Comparison — מדדים השוואתיים
                  </p>
                  <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
                    <table className="w-full text-xs">
                      <thead>
                        <tr style={{ background: 'rgba(255,255,255,0.04)' }}>
                          <th className="text-right px-3 py-2 font-bold" style={{ color: 'rgba(255,255,255,0.4)' }}>ספק</th>
                          <th className="text-center px-2 py-2 font-bold" style={{ color: 'rgba(255,255,255,0.4)' }}>תשואה/חודש</th>
                          <th className="text-center px-2 py-2 font-bold" style={{ color: 'rgba(255,255,255,0.4)' }}>יציבות רשת</th>
                        ...
                        {PROVIDERS.map(p => (
                          <tr
                            ...
                            style={{
                              ...
                              background: p.bestMatch ? 'rgba(139,92,246,0.08)' : 'transparent',
                            }}
                          >
                            <td className="px-3 py-2.5">
                              <div className="flex items-center gap-1.5">
                                <div className="w-2 h-2 rounded-full shrink-0" style={{ background: p.color }} />
                                <span className="font-bold" style={{ color: p.bestMatch ? '#C4B5FD' : 'rgba(255,255,255,0.85)' }}>
                                  {p.name}
                                </span>
                                {p.bestMatch && (
                                  <span className="text-[8px] px-1 rounded-full font-black"
                                    style={{ background: 'rgba(139,92,246,0.25)', color: '#C4B5FD' }}>★</span>
                                )}
                              </div>
                            </td>
                            <td className="text-center px-2 py-2.5 font-black" style={{ color: p.color }}>₪{p.monthlyYield}</td>
                            <td className="text-center px-2 py-2.5" style={{ color: 'rgba(255,255,255,0.5)' }}>{p.stabilityBonus}</td>
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-xs font-bold" style={{ color: 'rgba(255,255,255,0.45)' }}>
                      ניתוח אובייקטיבי – Yield Comparison עבור הפרופיל שלך
                    </p>
                  </div>
                  {analyzing ? (
                    <div className="flex items-center gap-2 py-2">
                      <Loader2 className="w-4 h-4 animate-spin" style={{ color: '#3B82F6' }} />
                      <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>מנתח נתוני SMP ומונה...</span>
                    </div>
                  ) : (
                ...
                {!switched ? (
                  <div className="space-y-2">
                    ...
                    >
                      {switching ? (
                        ...
                        <><Zap className="w-4 h-4" /> עבור ל-Cellcom · +₪{gap} בחודש</>
                      )}
                    </button>
                    <p className="text-[10px] text-center" style={{ color: 'rgba(255,255,255,0.3)' }}>
                      VPP Active Optimization יופעל · עמלת הצלחה 20% על רווח עודף בלבד
                    </p>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    ...
                    <span className="text-sm font-bold text-emerald-400">הבקשה נשלחה! VPP Active Optimization פעיל</span>
                  </div>
                )}
                ...
                >
                  ...
                  <p className="text-[10px] leading-relaxed" style={{ color: 'rgba(255,255,255,0.3)' }}>
                    VPP Solar Club is an independent aggregator. This diagnosis is mathematically derived from Noga Grid Data and is 100% provider-neutral.
                  </p>
                </div>
              </div>
            </motion.div>
